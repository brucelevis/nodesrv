...
static int port_dispatch(Port* port, int sockfd, const char* buf, int datalen)
{
    if(port->on_read) 
    {
        return port->on_read(sockfd, buf, datalen);
...
static void port_on_read(struct aeEventLoop *eventLoop, int sockfd, void *args, int event)
{
    ...
    for(;;)
    {
        ...
        LOG_DEBUG("real recv %d bufremain %d\n", ir, buflen);
        ...
        for (;;)
        {
            ...
            int packetlen = port_dispatch(port, sockfd, rptr, datalen);
            if (packetlen > 0)
            {
...
static void port_on_accept(struct aeEventLoop *eventLoop, int listenfd, void *args, int event)
{
    ...
    //LOG_INFO("%s a client connected ! port(%s) sockfd(%d) handler(%s)\n", port->name, port->name, sockfd, port->lua_on_accept);
    if (!port->on_accept && port->lua_on_accept[0] == 0)
    {
        ::close(sockfd);
        return;
    }
...
int listen(Port* port, unsigned short listenport)
{
    ...
    LOG_INFO("llisten on %d", listenport);
...
{   
    ...
    }else
    {
        ...
        //LOG_INFO("ip %s/n", inet_ntoa(*(struct in_addr*)(hostent->h_addr_list[0])));
