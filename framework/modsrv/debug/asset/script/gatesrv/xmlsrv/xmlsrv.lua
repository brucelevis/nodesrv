...
function ev_close(sockfd, reason)
    loginfo('client close from %s reason %s', Port.getpeerip(sockfd), reason)
...
function ev_accept(sockfd)
    loginfo('accept a client from %s', Port.getpeerip(sockfd))
    socket_manager[sockfd] = {sockfd = sockfd, time = os.time()}
end

function listen()
    local xml_srv = Config.xml_srv
    loginfo('listen on %s:%d', xml_srv.host, xml_srv.port)
...
function timer_check()
    ...
    for sockfd, player in pairs(socket_manager) do
        if timenow - player.time >= Config.tmp_sock_idle_interval then
            Port.close(sock_listen, sockfd, 'idle too long')
        end
    end
    return Config.tmp_sock_idle_interval * 1000
end

function main()
    listen()
    Timer.AddTimer(Config.tmp_sock_idle_interval * 1000, "ClientSrv.timer_check")
end
