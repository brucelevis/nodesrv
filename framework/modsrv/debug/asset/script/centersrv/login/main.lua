...
--功能:某个玩家上线
--@uid 玩家uid
function PLAYER_ENTER(sockfd, srvid, uid)
    ...
    if not player then
        loginfo('player online uid(%d)', uid)
        --插入一条记录
        player = {
                uid = uid,
    ...
    elseif player.srvid then
        loginfo('player instead to from game(%s) to game(%s)', game.srvname, player.srvname)
        --通知这个服下线
        POST(player.sockfd, 'Login.PLAYER_INSTEAD', uid, game.srvname)
...
--功能:某个玩家下线
--@uid 玩家uid
function PLAYER_EXIT(sockfd, srvid, uid)
    local game = Gamesrv.game_manager[srvid]
    if not game then
        logerr('srvid is not found srvid(%d)', srvid)
        return
    end
    loginfo('player offline uid(%d)', uid)
    local player = player_manager[uid]
    local srvid = game.srvid
    if not player then
