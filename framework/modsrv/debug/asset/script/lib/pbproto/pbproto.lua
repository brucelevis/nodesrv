...
function _decodebuf(sockfd)
    ...
    --loginfo('real_recv(%d) bufremain(%d)', real_recv, bufremain)
    local datalen = Recvbuf.datalen(sockfd) 
    if datalen < 2 then
        --loginfo('header not enough(%d)', datalen)
        return
    end
    local plen = Recvbuf.getint16(sockfd)
    --loginfo('plen(%d)', plen)
    if Recvbuf.datalen(sockfd) < plen then
        --loginfo('data not enough(%d/%d)', datalen, plen)
        return
    end
    ...
    --loginfo('plen(%d) seq(%d) msgname(%s) msglen(%d)', plen, seq, msgname, msglen)
    Ar.free(arfd)
...
function decode(sockfd)
    ...
    loginfo('[PB][RECV] msgname(%s)', msgname)
...
function send(sockfd, msg)
    ...
    loginfo('[PB][SEND] plen(%d) msgname(%s) msglen(%d)', plen, msgname, msglen)
    return plen
end
