...
function main()
    --构建descriptor
    for table_name, table_conf in pairs(argv.table_conf) do
...
local function select_from_mysql(uid, table_name)
    ...
    local pb_class = argv.table_conf[table_name].class
...
function get_from_redis(sockfd, uid, callback, ...)
    ...
    if redis_reply and redis_reply.type == 'array' then
        for idx, v in pairs(redis_reply.value) do
            ...
            if v.type == 'string' then
                local msg = Pblua.msgnew(argv.table_conf[table_name].class)
    ...
    Redis.command(conn, string.format('EXPIRE %d %d', uid, argv.expire_sec))
...
function set_to_redis(sockfd, uid, callback, ...)
    ...
    --EXPIRE
    local redis_reply = Redis.command(conn, string.format('EXPIRE %d %d', uid, argv.expire_sec))
...
function set_to_mysql(sockfd, uid, callback, ...)
    ...
    for i = 1, #args, 2 do
        ...
        local msg = args[i + 1]
        --是否要展开这个表
        local pb_class = argv.table_conf[table_name]
