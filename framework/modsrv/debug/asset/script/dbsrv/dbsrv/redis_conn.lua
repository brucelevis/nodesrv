...
function check_redis_connections()
    ...
    for index, info in pairs(redis_table) do
        local conf = argv.redis_conf[index]
        if not conf or conf.host ~= info.host or conf.port ~= info.port or conf.dbname ~= info.dbname then
            loginfo('close redis connection index(%d) host(%s) port(%d) dbname(%s)', index, conf.host, conf.port, conf.dbname)
            Redis.close(info.redis)
            redis_table[index] = nil
        end
    end
    --连接
    for index, conf in pairs(argv.redis_conf) do
        if not redis_table[index] then
            local conn = Redis.connect(conf.host, conf.port)
            if not conn then
                loginfo('fail connect redis index(%d) host(%s) port(%d) dbname(%s)', index, conf.host, conf.port, conf.dbname)
                redis_table[index] = nil
            else
                loginfo('success connect redis index(%d) host(%s) port(%d) dbname(%s)', index, conf.host, conf.port, conf.dbname)
    ...
    if not reply or reply.value ~= 'PONG' then
        for index, info in pairs(redis_table) do
            if info.conn == conn then
                ...
                redis_table[index] = nil
                loginfo('ping fail, close redis connection index(%d) host(%s) port(%d) dbname(%s)', index, conf.host, conf.port, conf.dbname)
                break
            end
        end
    end
