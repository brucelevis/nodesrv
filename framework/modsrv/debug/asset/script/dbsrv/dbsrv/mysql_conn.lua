...
function check_mysql_connections()
    ...
    for index, info in pairs(mysql_table) do
        local conf = argv.mysqls_conf[index]
        if not conf or conf.host ~= info.host or conf.dbname ~= info.dbname or conf.user ~= info.user then
            loginfo('close mysql connection index(%d) host(%s) dbname(%s)', index, conf.host, conf.dbname)
            Mysql.close(info.conn)
            mysql_table[index] = nil
        end
    end
    --连接
    for index, conf in pairs(argv.mysql_conf) do
    ...
    if not reply then
        for index, info in pairs(mysql_table) do
            if info.conn == conn then
                ...
                loginfo('ping fail, close mysql connection index(%d) host(%s) dbname(%s)', index, conf.host, conf.dbname)
                break
            end
        end
    end
